...
| T40 | . | (stabilization gate) isolate test config from developer `.env`: set test env at `tests/conftest.py` import — before any `app.config` import — (dummy `COACH_TOKEN`, unset `NOTION_API_TOKEN`/`OPENAI_API_KEY`, test DB URL) and/or rebuild the `app.config.settings` singleton; seed-or-skip AAMC outline so `OutlineLookup` resolves (⊥ implicit startup seed — V-O6; fixture materializes via importer or test skips). An autouse fixture alone is too late (singleton built at import). Gate: clean-checkout `uv run pytest` 27 failed/21 errors → 0 | V-TC1,V16,V-O6 |
