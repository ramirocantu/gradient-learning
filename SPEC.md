...
| T45 | . | (client-unblock, desktop ¶T8) `GET /api/v1/concept-edges` read API (recent edges + by-node): expose `concept_edges` (`from`/`to` node, `kind`, `score`, `created_at`) for the connections feed. Dep: P2 substrate (T24) + similarity-edge derivation (T28) | V-E2,V-O1,V-D1 |
| T46 | . | (client-unblock, desktop ¶T9) `GET /api/v1/atomic-facts` read API (by node / by pdf): expose `atomic_facts` (`text`, `node_id`, `pdf_source`, `page`, `extractor_version`). Dep: P2 substrate (T24) + grounded extraction/persist (T26/T30) | V-KB1,V-T1,V-D1 |
| T47 | . | (client-unblock, desktop ¶T10) `GET /api/v1/notion/pages` read API: expose the `notion_pages` pointer index (`node_id`, `title`, `url`, `block_count`, `last_synced`, `status`) for the page-index view. Read of OUR pointer index per V-N1 (⊥ Notion read-back); one page per node (V-N2). Dep: P2 substrate (T24) + Notion write-out (T32) | V-N1,V-N2,V-D1 |
| T48 | . | (client-unblock, desktop ¶T11) `GET /api/v1/pdf-sources` read API (inbox): expose `pdf_sources` (`filename`, `pages`, `status`, `facts_count`, `ingested_at`, `node_id`, `sha`). Dep: P2 substrate (T24) + PDF ingest seam (T26) | V-KB1,V-D1 |
