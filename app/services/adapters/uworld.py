...
Holds the capture→{Question, Attempt} normalization that the ingest endpoint
ran inline before T3. Registered under `source='uworld'`; the generic
`ingest_capture` dispatcher routes here by `payload.source`. Runs entirely
inside the caller's transaction — the endpoint owns commit/rollback.
"""
